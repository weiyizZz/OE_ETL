...
pipeline_type="refiner_2nd"
DB_PATH = "DB/oedb_refiner_2nd_r4.db"
...
prompt_path_refiner_ParReducer = "data/prompt_templates/refiner/prompt_refiner_ParReducer.yaml"
...
notegroup_id_start = int(input("Enter starting notegroupID: ").strip())
notegroup_id_end = int(input("Enter ending notegroupID (inclusive): ").strip())

for notegroup_id in range(notegroup_id_start, notegroup_id_end + 1):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        row = cursor.fetchone()
        if row is None:
            print(f"--- Skipping notegroupID {notegroup_id}: not found in notegroups ---")
            continue
    ...
    if has_participant:
        ...
        reducer = ParticipantReducer(
            ...
            prompt_path_refiner_ParReducer=prompt_path_refiner_ParReducer,
        )
    ...
    transformer_2json = Text2JsonTransformer(prompt_path_text2json=prompt_path_text2json,
                                             ...
                                             pipeline_type=pipeline_type, prompt_path_refiner_pqa=prompt_path_refiner_pqa,
                                             prompt_path_refiner_1recordT=prompt_path_refiner_1recordT)
